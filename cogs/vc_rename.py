...
class VCRename(Cog):
    ...
    async def vc(self, ctx: Context, *, name: str):
        ...
        if not state:
            ...
            return
        if state.channel.id == const.CH_AFK:
            await ctx.send(f"{ctx.author.mention} AFKチャンネルに接続中は実行できません。")
            return
        if ctx.channel.id != const.VOICE_CHANNELS[str(state.channel.id)]["vc_text"]:
            await ctx.send(f"{ctx.author.mention} VCに対応したチャンネルで実行してください。")
            return
        if not ctx.channel.id in [
            value.get("vc_text") for value in const.VOICE_CHANNELS.values()
        ]:
    ...
    async def on_voice_state_update(self, member, before, after):
        ...
        if not str(vc.id) in const.VOICE_CHANNELS.keys():
            return
        name = const.VOICE_CHANNELS[str(vc.id)]["name"]
        if len(vc.members) == 0 and vc.name != name:
            await vc.edit(name=name)
            vc_text = self.bot.get_channel(
                const.VOICE_CHANNELS[str(vc.id)]["vc_text"]
            )
